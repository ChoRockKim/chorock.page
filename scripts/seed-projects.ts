...
기존 학교 앱은 정보 조회에는 충분하지만 매일 자발적으로 열어볼 이유가 없습니다. 그래서 접속 → 부 상태 확인 → 학식/퀴즈 → XP 성장 → 진화 → 마이룸 확장이라는 하루 루프를 먼저 설계하고, 그 위에 기능을 붙였습니다.

## 맡은 일

Expo(React Native) 앱 전체입니다. 기획·디자인·백엔드는 팀원이 맡았고, 저는 화면과 게임 로직, 상태 설계, 서버 API 연동, 실기기 빌드/테스트를 담당했습니다. 백엔드는 FastAPI(JWT, 9자리 학번 로그인, \`@hufs.ac.kr\` 메일 인증으로 외대 구성원만 가입)였고, 프론트를 붙이면서 계약이 어긋나는 지점을 문서로 정리해 백엔드에 넘겼습니다.

## 주요 기능

- **성장 루프** — 학식과 퀴즈로 XP를 쌓아 새내기부터 졸업까지 진화하고, 임계치 아래로 떨어지면 유급됩니다. 학년별 XP를 따로 저장하지 않고 누적 XP(\`totalXp\`) 하나만 두고 학년과 진행률은 클라이언트에서 계산합니다. 서버가 붙은 뒤에도 이 구조를 그대로 썼습니다.
- **학식·퀴즈** — 조·중·석식 시간대별 먹이기 제한과 끼니를 거르면 XP가 깎이는 공복 시스템, 학사·캠퍼스 정보를 내는 O/X·객관식 퀴즈(하루 3문제, 쿨타임).
- **미니게임 3종** — 전공책 받기, 부 잡기, 자유투 넣기. 하트를 소모해 시작하고 성공하면 코인을 받으며 랭킹은 전체/친구로 나뉩니다.
- **마이룸** — 코인으로 가구·벽지 구매와 배치, 친구 방 방문과 방명록.
- **친구·업적·졸업 리포트**, 게스트 모드, 픽셀 아트 UI(팀에서 Figma로 제작).

## 문제 해결

### 행동 하나가 여러 도메인으로 번지는 상태 관리

화면만 있는 앱이 아니라 성장 게임이라, 기능이 늘수록 사용자 행동 하나가 여러 도메인으로 연쇄됩니다.

- 퀴즈 정답 → XP 증가 → 학년 상승 → 진화·졸업 화면 → 서버 통계 반영
- 미니게임 성공 → 하트 차감 검증 → 결과 저장 → 코인 보상 → 랭킹 반영

초기에는 로컬 데이터 중심으로 빠르게 만들었는데, 서버 API가 붙으면서 이 연쇄의 단계마다 **무엇을 권위 있는 값으로 볼 것인가**를 다시 정해야 했습니다. 즉시 반응하는 UI는 필요하지만, 코인·하트·구매처럼 서버 권위가 중요한 영역까지 그렇게 처리하면 데이터가 틀어집니다. 아래 판단들이 전부 여기서 갈라져 나왔습니다.

### 로컬 우선으로 만든 앱에 서버를 붙이면서 "무엇이 진실인가"를 다시 정한 일

개발 초기에는 속도를 위해 로컬 더미 데이터로 기능을 먼저 만들었습니다. 문제는 서버를 붙이는 시점이었습니다. 로그인 상태인데도 로컬 더미가 보이면, **서버가 데이터를 안 주고 있는 건지 잘 주고 있는 건지 구분할 수가 없습니다.** 장애가 더미에 가려집니다.

그래서 로그인 상태에서는 로컬 fallback을 대부분 차단하고, 핵심 데이터가 비면 화면이 의도적으로 실패를 드러내도록 뒀습니다. 대신 서버 없이 넓게 테스트해야 하는 기능을 위해 로컬과 서버를 완전히 분리한 **게스트 모드**를 따로 만들었습니다.

이 원칙 덕분에 실제 장애가 그대로 드러났습니다. 마이룸 가구 구매가 안 되던 건은 프론트 버튼 문제가 아니라 서버 \`/shop/items\`가 200으로 **빈 배열**을 주고 있었던 것이었고, 로그를 근거로 백엔드에 전달했습니다. 신규 유저의 캐릭터 동기화 403도 같은 방식으로 잡았습니다.

### 기기를 바꾸면 무엇이 사라지는지 먼저 적어보고 고친 일

Zustand persist로 로컬에 쌓이는 값이 늘면서, 새 기기에서 로그인하면 무엇이 어긋나는지 불안해졌습니다. 증상을 기다리는 대신 **persist 상태를 전부 나열하고 서버에서 복구 가능한 것과 아닌 것으로 갈라 문서로 정리했습니다.**

위험한 항목이 분명해졌습니다. 끼니 누락 횟수와 이미 적용한 XP 패널티 수가 로컬에만 있어서, 기존 기기에서는 배고픈 상태였는데 새 기기에서는 멀쩡해 보이고 패널티도 기기마다 다르게 적용될 수 있었습니다. 퀴즈 일일 카운터도 마찬가지였고, 둘 다 XP·코인이 걸린 값이라 경제 밸런스로 이어집니다.

고친 방향은 권위를 서버로 옮기는 것이었습니다. 로그인 상태에서는 서버의 먹이기 상태와 퀴즈 가능 여부만 기준으로 삼고, 로컬 값은 비로그인 fallback 전용으로 격리했습니다.

### 낙관적 업데이트를 어디까지 쓸지 선을 그은 일

친구 요청 수락, 방명록 작성, 가구 장착처럼 단순한 동작조차 서버 응답을 기다리면 체감이 확 나빠집니다. 그래서 일단 성공했다고 가정하고 화면을 먼저 바꾼 뒤 실패하면 되돌리게 했습니다.

다만 **전부 그렇게 하지는 않았습니다.** 하트 차감·상점 구매·미니게임 시작처럼 코인과 소유권이 걸린 작업은 낙관 처리하면 실패를 숨기게 되고 정합성이 깨집니다. 이런 경로는 로딩 오버레이로 진행 상황을 보여주고 서버 성공을 권위값으로 삼았습니다. 보상과 결과 저장이 분리되어 있어 세션 id 기준 멱등 처리도 함께 필요했습니다.

### 게임 라이브러리가 아닌 도구로 미니게임을 만든 일

React Native는 게임 엔진이 아닙니다. 전공책 받기나 부 잡기는 예상보다 수월했지만, 자유투는 물리적 상호작용이 필요해서 애를 먹었습니다. 물리 엔진을 도입했는데 공이 의도대로 움직이지 않았습니다.
...
슛 순간 게이지 바가 왼쪽으로 튀던 버그도 있었는데, 슛 애니메이션이 마커 진행 애니메이션을 리셋시키는 게 원인이라 누른 순간의 값을 붙잡아 고정 렌더링하도록 고쳤습니다.

### 실기기에서만 확인되는 앱인데, 정작 기기에 올릴 방법이 마땅치 않았던 일

효과음 타이밍, 에셋 로딩, Android 하단 내비게이션 바에 UI가 잘리는 문제 같은 건 시뮬레이터로는 판정이 안 되고 실기기에서만 드러납니다. 그런데 그 실기기에 앱을 올릴 경로가 없었습니다. 유료 애플 개발자 계정이 없어 iOS는 **유선 설치만** 가능했고, Android도 스토어에 올리지 않아 한 줄짜리 오류를 고쳐도 APK를 다시 빌드해 다시 설치해야 했습니다. 고치는 시간보다 확인하는 시간이 더 드는 구간이 계속 생겼습니다.

### 에셋 프리로드 시점 조절

이미지·아이콘·BGM·효과음이 많아서, 배경이 로드되기 전에 진입하면 빈 화면이 잠깐 보였습니다. 로딩 오버레이로 시간을 확보하고 백그라운드 프리로드를 도입했는데, **어려운 건 기술이 아니라 시점 선택이었습니다.** 한 번에 많이 불러오면 진입이 느려지고 적게 불러오면 빈 화면이 남습니다. 결국 화면마다 critical asset만 await하고 나머지는 화면을 띄운 뒤 백그라운드로 밀었습니다.

## 검증

기능이 도는지가 아니라 **목표를 달성했는지**를 확인하는 쪽으로 테스트를 짰습니다.

- **기능 테스트** — 54개 시나리오를 Pass/Fail/Unstable로 판정해 **51 Pass, Unstable 3, Fail 0**. Unstable 3건은 전부 로딩 지연이었고(캠퍼스 건물 이동, 미니게임 시작, 가구 구매) 감추지 않고 남겼습니다.
- **사용성 테스트** — 한국외대 재학생·휴학생·졸업생 **20명** 설문(2026.06.03–09). 평균 만족도 4.6, 정보 습득에 도움 93%, 학교 생활 흥미 상승 65%.
- **성능 테스트** — Locust 부하 테스트. 동시 50명까지 총 3,546건 요청에 21 RPS, 평균 응답 342.74ms, **실패율 0%**.
...
- 스토어에 배포하지 않았습니다. 실제 사용자에게 전달된 적이 없다는 뜻입니다.
...
\`app / features / shared\` 3계층 폴더 규칙과 커밋·브랜치 규칙을 문서 한 파일에 고정하고, 작업 단위마다 "무엇을 했나 · 남은 작업 · 잠재 버그"를 로그로 남겼습니다. 아래 문제 해결 두 건 모두 그 기록에서 나왔습니다.
...
- 보존지수 — 셀카 분석과 체크리스트 완료 후 시술별로 산출. 반원 게이지와 변화 이력으로 표시합니다.
...
그래서 앱 안에 임시 목 백엔드를 두고 전 플로우를 끝까지 밟을 수 있게 했습니다. 상태는 AsyncStorage에 저장해 앱을 다시 켜도 유지되고, 모든 호출에 지연을 넣었습니다(일반 400ms, 인증 700ms, 셀카는 업로드 1.2초 + 분석 3초). 지연은 로딩 표시와 버튼 비활성 처리가 실제로 눈에 보이게 하려는 것이었습니다 — 즉시 응답하는 목은 그 두 가지를 검증하지 못합니다.

교체 비용을 미리 줄여두는 게 핵심이었습니다. 조회는 \`shared\`의 셀렉터에, 쓰기는 각 도메인의 \`api/\` 파일에 몰아둬서 실제 API가 붙을 때 **구현만** 바꾸면 화면 코드는 손대지 않아도 되게 했습니다. 폐기 절차도 문서에 같이 적어뒀습니다 — 임시 코드는 지우는 방법까지 적어두지 않으면 남습니다. 실제로 서버가 열린 뒤 도메인별로 교체했고, 화면 수정은 응답 계약이 어긋난 한 곳에서만 필요했습니다.
...
**앞의 두 번은 제가 잘못된 기준을 골라서 틀렸습니다.** 1차로 "iOS는 픽셀을 회전하지 않고 EXIF에만 방향을 적는다"고 보고 EXIF 기준으로 회전시켰지만 실패했고, 2차로 캡처 버퍼가 가로로 고정된 것이라 보고 판별 기준을 프리뷰 방향으로 바꿨는데 이 변경 자체는 옳았지만 증상은 그대로였습니다.
...
고칠 곳은 회전량 계산이 아니라 파일을 넘기는 방식이었습니다. 회전량이 0이어도 Skia로 디코딩한 결과를 **새로 인코딩**해서 넘기게 하니 방향이 픽셀에 구워지고 EXIF 태그가 사라집니다. 실기기에서 "0회전에서 검출 성공"을 확인했습니다. 회전 방향은 메타데이터만으로 확정할 수 없다는 걸 두 번 겪은 셈이라, 첫 시도에서 못 찾으면 남은 방향까지 순서대로 시도하는 폴백도 뒀습니다.

여기서 배운 건 **진단 로그를 먼저 넣었어야 한다**는 것입니다. 로그가 없어서 두 번을 헛짚었고, 넣자마자 한 줄로 끝났습니다. 같은 뿌리에서 조용히 틀려 있던 것도 하나 더 나왔습니다 — 밝기를 재는 영역이 엉뚱한 데를 재고 있었는데, 우연히 중앙 근처라 값이 그럴듯해서 들키지 않고 있었습니다.
...
**추측으로 끝내지 않고 컴파일 결과를 직접 확인했습니다.** babel에 \`supportsReactCompiler: true\`를 주고 산출물을 비교했더니 의존성 비교가 \`$[3] !== form\`에서 \`$[3] !== form || $[4] !== isValid\`로 바뀌는 것이 보였습니다. \`useFormState()\`로 받으면 원시값이라 의존성에 잡히는 것이 확인된 셈입니다.

회원가입 훅도 같은 문제였는데 이메일 유효 여부가 뒤집힐 때만 우연히 메모가 갱신돼서, **이메일을 먼저 채우면 그 뒤로 폼이 동결되는** 형태로 숨어 있었습니다. 입력 순서에 따라 재현되기도 하고 안 되기도 했던 이유입니다. 테스트는 컴파일러를 거치지 않은 코드를 돌리기 때문에 원리적으로 못 잡는 종류의 버그였습니다.

## 성과와 한계

- 프론트엔드를 혼자 맡아 화면 15개와 서버 연동까지 마쳤습니다. 코드 커밋은 2026년 8월 11일부터 21일까지 114건이고, 저장소 세팅 커밋을 빼면 프론트엔드 커밋은 전부 제 것입니다.
...
- 작업 로그가 8월 18일에서 멈춰 있습니다. 19~21일의 서버 연동 작업은 커밋 메시지에만 남았고, 마감이 가까워질수록 기록이 가장 먼저 밀렸다는 뜻입니다.
