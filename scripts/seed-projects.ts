...
- 에이전트만으로 MVP를 만든 것은 이번이 처음이라 많이 미숙했습니다. 에이전트를 협업에 쓰는 방식도 서툴렀고, 초반 정책을 정하는 데 시간을 꽤 썼는데 그게 효율적이었는지는 지금도 의문입니다. 같은 시간이 주어졌는데도 팀마다 결과물 퀄리티 차이가 꽤 났고, 에이전트를 잘 쓰는 방법을 다시 생각해보게 된 계기가 됐습니다.
- 다음 단계는 차근차근 실사용자를 모아보는 것입니다.
...
### 생성이 영구히 막히던 경로를 없앤 일

작업 행을 넣은 뒤 저장소 연결 삽입이 실패하면 보상 없이 예외만 던지고 있었습니다. 활성 작업은 사용자당 하나만 허용되는데, 남겨진 대기 행이 그 사용자의 다음 생성을 **영원히** 막았습니다. 재시도는 실패 상태에서만 가능해서 스스로 풀 방법도 없었습니다. 삽입이 실패하면 방금 넣은 행을 되돌리도록 고쳤습니다.

프로세스가 중간에 죽는 경우까지는 보상으로 못 막습니다. 그래서 오래 진척이 없는 작업을 실패로 돌리는 청소를 넣었는데, 문제는 실행 위치였습니다 — 무료 플랜의 크론은 하루 한 번뿐이라 리퍼로 쓸 수 없었습니다. 대신 막힌 사용자가 반드시 지나가는 두 지점, 생성 요청 진입부와 진행 화면이 2초마다 두드리는 조회에서 청소를 돌렸습니다. 서버 어디에도 외부 호출 시간 제한이 없던 것도 이때 함께 고쳤습니다 — 매달린 작업 하나가 다음 시도까지 막는 구조였기 때문입니다.

### 실패가 전부 같은 문장으로 뭉개지던 문제

생성이 실패하면 원인이 무엇이든 "잠시 후 다시 시도해주세요" 한 문장이었습니다. GitHub 호출 한도에 걸린 사람과 연동이 끊긴 사람이 같은 화면을 보고, 둘 다 즉시 재시도를 눌러 같은 실패를 반복했습니다. 에러를 원인별로 분류해 사유를 보여주고, 다시 해도 결과가 같을 실패는 그 자리에서 끝내게 했습니다. 진행 화면 폴링도 한 번만 실패하면 즉시 멈춰서 네트워크가 잠깐 끊기면 멀쩡한 작업이 실패한 것처럼 보였는데, 세 번까지 간격을 늘려 기다리게 했습니다.

이 결함들이 오래 남아 있던 이유가 핵심입니다 — **목 백엔드가 한 번도 실패하지 않아서, 실패 경로는 로컬에서 실행될 일이 없었습니다.** 그래서 목에 실패를 주입하는 스위치를 만들어, 이제 실패 화면도 로컬에서 개발하고 확인할 수 있습니다.

### 남의 커밋이 지원자의 성과로 서술될 뻔한 문제

커밋과 PR을 저자 구분 없이 생성 근거로 넘기고 있었습니다. 팀 저장소라면 팀원의 작업이 지원자의 성과로 서술될 수 있는 구조입니다 — 포트폴리오 생성 서비스가 가장 해서는 안 되는 실수입니다. 응답에 이미 들어 있던 저자 정보로 본인 작업과 팀 작업을 나눠 담고, 팀 작업은 맥락으로만 쓰라는 지침을 더했습니다. 추가 API 호출은 없습니다.

같은 이유로 근거에 없는 기술이 결과에 섞이면 걸러냅니다. 프롬프트로 금지해도 모델은 그럴듯한 이름을 덧붙이는데, **면접에서 설명하지 못할 한 줄이 남는 쪽이 더 큰 손해**이기 때문입니다. 무엇을 걸렀는지는 로그로 남겨 프롬프트를 고칠 근거로 씁니다.
...
`,
  ...
  },
  {
    ...
    period: "2026.03 — 현재",
    tags: ["React Native", "Expo", "TypeScript", "Tanstack Query", "Expo Router"],
    stack: [
      {
        label: "앱",
        items: ["Expo SDK 54", "React Native 0.81", "React 19", "TypeScript", "Expo Router v6"],
      },
      {
        label: "상태·네트워크",
        items: ["Tanstack Query", "Axios", "expo-secure-store", "React-Hook-Form"],
      },
      {
        label: "인증·네이티브",
        items: ["네이버 로그인 SDK", "expo-apple-authentication", "expo-notifications", "expo-updates"],
      },
      { label: "운영", items: ["Firebase Analytics", "Google Mobile Ads"] },
    ],
    coverImage: "/projects/fora/forA-thumbnail.webp",
    coverImageFit: "contain",
    overviewMd: `국내 최초의 ADHD 커뮤니티 앱입니다. 커뮤니티, 약 검색·리뷰, 건강 매거진을 갖춘 헬스케어 앱으로, Expo(React Native) 한 코드베이스로 iOS·Android를 함께 빌드하며 앱스토어와 구글 플레이스토어에서 서비스 중입니다. 2026년 3월부터 합류해, 전임 개발자가 초기 구축을 마쳐둔 코드베이스를 인수인계받아 프론트엔드 개발과 유지보수를 맡고 있습니다.

## 맡은 일

- 창업팀의 기획자·디자이너·백엔드 개발자와 한 팀으로 일합니다. 서버 API는 백엔드가, 화면 설계는 디자이너가 맡고, 저는 앱 구현과 스토어 릴리즈를 담당합니다.
- 인수인계 이후 제가 붙인 주력 영역은 **소셜 로그인 전 과정**과 **건강 매거진 전반**입니다. 커뮤니티(게시글·댓글·신고·차단)는 기존 구현을 이어받아 보수하고 있습니다. 약 검색·리뷰, 푸시 알림, 광고·분석은 전임자가 만든 영역입니다.
- 소셜 로그인은 백엔드 연동 문서 작성부터 시작해 SDK 연동, 서버 배선, 가입·계정 연결 화면, 엣지케이스 점검, 네이버 사전 검수 준비까지 맡았습니다.
...
- 네이버·애플 소셜 로그인을 출시 가능한 상태까지 끝냈습니다. 클라이언트 연동부터 서버 연동, 엣지케이스 대응, 사전 검수 준비까지 마쳤습니다.
- 매거진 기사 발행과 오늘탭 배너 운영이 개발자를 거치지 않게 되면서, 콘텐츠 운영이 개발 일정에서 분리됐습니다.
- 1.5.9 버전을 OTA로 배포했습니다.
- 테스트 코드가 없습니다. 작업 규칙 문서에 검증 기준은 세워뒀지만 자동화된 테스트는 아직 없어서, 회귀 확인이 실기기 수동 검증에 의존합니다.

## 주요 기능

- 소셜 로그인 — 네이버·애플. 로그인, 신규 가입(2단계), 기존 계정 연결 세 갈래를 각각 화면으로 처리합니다.
- 계정 관리 — 소셜 전용 계정에는 비밀번호 변경 메뉴를 감추고, 탈퇴 시 소셜 세션까지 함께 정리합니다.
- 매거진 관리자 작성 — 관리자가 앱 안에서 직접 기사를 쓰고 발행합니다. 발행 전 미리보기, 썸네일·본문 이미지 업로드, 발행 알림을 함께 붙였습니다.
- 매거진 댓글·대댓글 — 작성·수정·삭제·좋아요. 작성자는 실명 대신 익명 1·익명 2로 노출하고, 삭제된 댓글은 감추되 대댓글이 남아 있으면 안내 문구만 남깁니다.
- 매거진 탐색 — 인기순·24시간 급상승 카테고리, 조회수와 좋아요 수 표시. 본문 이미지는 누르면 확대되고 아래로 드래그하면 닫힙니다.
...
소셜 로그인은 백엔드 계약 회신을 기다려야 했습니다. 앱 코드를 아예 멈추고 기다릴지, 서버와 무관한 부분부터 할지 골라야 했습니다.

네이티브 설정과 prebuild가 가장 오래 걸리고 가장 잘 깨지는 구간입니다. API가 확정된 뒤에 시작하면 그때부터 일정이 통째로 밀립니다. 그래서 SDK 연동과 네이티브 설정을 먼저 끝내고 서버 요청부만 비워뒀습니다. 나중에 서버 엔드포인트가 올라왔을 때 배선만 이으면 됐습니다.
...
이후 서버 명세가 개편됐을 때도 명세서만 믿지 않고 **서버 코드로 직접 대조**했습니다. 실제로 명세와 구현이 다른 곳이 있었습니다.
...
소셜 로그인 배선을 끝내고 보니, 서버 응답의 세 갈래 중 두 갈래가 **토스트만 띄우고 끝났습니다.**

신규 소셜 사용자는 서버에 프로필이 없어 권한이 GUEST입니다. 사용자 조회 API가 403을 주기 때문에 앱을 껐다 켜도 로그인 화면을 벗어날 수 없었습니다. 막다른 길이었습니다. 기존 계정과 연결해야 하는 갈래도 마찬가지였습니다 — 서버 엔드포인트 네 개 중 계정 연결만 앱에서 부르는 곳이 없었고, 훅은 있는데 호출처가 없는 상태였습니다.

두 갈래에 각각 화면을 붙였습니다. 신규 가입은 닉네임과 약관을 2단계로 나눠 받고, 계정 연결은 기존 계정 확인 후 연결하도록 했습니다.

이후 전 경로를 훑어 엣지케이스를 전수 점검했습니다. 대부분은 이미 막혀 있었고 구멍이 6개였습니다. 다시 조사하지 않도록 대응 표로 남겼습니다.

### 애플에서 연동을 끊어도 로그인 상태로 남던 문제

사용자는 iOS 설정에서 언제든 애플 로그인 연동을 끊을 수 있습니다. 그런데 애플 credential만 폐기되고 서버 토큰은 그대로 살아 있어서, 앱은 계속 로그인 상태로 보였습니다. 사용자 입장에서는 "애플에서 끊었는데 왜 아직 들어가지지"가 됩니다.

앱이 켜질 때 애플 credential 상태를 확인해, 폐기됐으면 로그아웃하도록 했습니다.

비슷한 결의 문제가 탈퇴에도 있었습니다. 네이버로 로그인한 뒤 탈퇴하고 다시 네이버 버튼을 누르면 계정 선택 없이 바로 로그인됐습니다. 로그아웃과 탈퇴가 각자 세션을 정리하고 있었던 게 원인이라, 정리 경로를 한 곳으로 모았습니다.

### 매거진 본문 글자가 잘리던 문제

기사 본문에서 특정 줄의 글자가 잘려 보였습니다. 4월과 5월에 두 번 우회했고 두 번째 커밋 제목에는 "최종 수정"이라고 적었는데, 최종이 아니었습니다.

같은 자리를 두 번 우회했는데도 재발한다는 건 개별 케이스가 아니라 렌더러 자체의 문제라는 신호로 봤습니다. 확인해보니 그 마크다운 라이브러리는 유지보수가 곧 끊길 예정이기도 했습니다. 세 번째로 우회해도 다음 SDK 업그레이드에서 다시 막힐 상황이라, 호환성이 좋은 라이브러리로 교체했습니다. 이후 같은 증상은 재발하지 않았습니다.

### 애플 재로그인 때 이름·이메일이 null로 오던 문제

로그인은 되는데 이름과 이메일이 비어서 왔습니다. 연동 코드를 의심하기 딱 좋은 증상이었습니다.

실기기로 1회차와 2회차 로그인을 나눠 확인했습니다. 1회차에는 두 값이 정상이었고 2회차부터 둘 다 null이었으며, 사용자 식별자는 두 번 다 동일했습니다. 즉 코드 문제가 아니라 애플이 개인정보를 최초 1회만 내려주는 정책이었습니다.

문서로만 알던 전제를 실측으로 확정한 셈이고, 이 사실이 서버 설계로 이어졌습니다 — 서버가 첫 응답을 저장하지 못하면 재로그인으로는 복구할 방법이 없으므로, 최초 응답 저장을 필수 조건으로 못 박았습니다.

### 글꼴이 조용히 시스템 폰트로 떨어지던 문제

안드로이드와 iOS에 같은 매거진 화면을 나란히 띄워 비교하다 발견했습니다. 같은 글자 크기인데 줄바꿈 위치가 달랐습니다.

Pretendard SemiBold의 등록 이름이 실제 사용처와 어긋나 있었고, 그 결과 **89곳이 시스템 폰트로 폴백**되고 있었습니다. 에러가 나지 않으니 아무도 몰랐고, 두 플랫폼을 나란히 놓기 전까지는 눈에도 띄지 않았습니다. 이름을 맞추고, 마크다운 본문이 별도 경로로 시스템 폰트를 쓰던 것도 함께 고쳤습니다.
  ...
  },
{
    ...
    role: "프론트엔드 (단독)",
    team: "해커톤 팀 5명 — 프론트엔드 1명(본인) · 백엔드 3명 · 기획/디자인 1명",
    period: "2026.08",
    tags: [
      "React Native",
      "Expo",
    ...
    coverImageFit: "cover",
    overviewMd: `피부 시술을 받은 뒤의 관리 기록을 한곳에 모으는 앱입니다. 시술 이력을 등록하고 셀카 체크인과 데일리 체크리스트를 남기면 "보존지수"와 오늘의 케어, 재시술 D-day를 보여줍니다. **멋쟁이사자처럼 애니멀리그 중앙해커톤**에 나가 만든 팀 프로젝트이고, 저는 프론트엔드를 전담했습니다. 서버는 팀의 Django/DRF API를 붙였습니다.
