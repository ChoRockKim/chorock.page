...
- 네이버·애플 소셜 로그인을 **1.6.0 버전으로 앱스토어·구글 플레이스토어에 출시했습니다**(2026.09). 백엔드 연동 문서부터 네이버 사전 검수 승인, App Store 심사 대응까지 전 과정을 맡았습니다. 3개월 반 만의 네이티브 릴리즈였습니다.
...
1차 진단은 낡은 토큰이 재로그인 요청에 붙어 401 재시도 루프에 빠진다는 것이었는데, **로그를 보지 않고 코드만 읽고 세운 가설**이었습니다. 실기기 로그를 받아보니 401이 한 번도 없었습니다. 2차 진단은 서버가 요구사항을 반쪽만 반영한 버그라는 것이었는데, 이번엔 **참조 문서만 읽고 작업 이력을 보지 않은** 게 문제였습니다. "이메일 없이도 가입이 성립해야 한다"는 계약이 문서 통합 과정에서 삭제돼 참조 문서에서 사라져 있었고, 작업 로그에만 남아 있었습니다.

3차에서야 전체가 보였습니다. 이메일 없는 소셜 계정을 GUEST로 두고 마이페이지에서 이메일을 추가하면 승격시키는 의도된 구조인데, GUEST는 사용자 조회가 거부돼 로그인 자체가 안 되니 마이페이지에 갈 수 없었습니다. **이메일을 추가해야 로그인이 되는데, 추가하는 화면이 로그인 뒤에 있는 닫힌 고리**였습니다. 가입 흐름 안에 이메일 인증을 넣어 풀었고, 백엔드에 승격 기준 변경을 제안해 채택되면서 같은 날 그 우회를 걷어내고 이메일 등록을 설정 화면으로 옮겼습니다.
...
글꼴도 같은 부류였습니다. 안드로이드와 iOS에 같은 화면을 나란히 띄우니 줄바꿈 위치가 달랐고, 추적해 보니 Pretendard SemiBold의 등록 이름이 어긋나 **89곳이 시스템 폰트로 폴백**되고 있었습니다. 에러가 나지 않는 어긋남이라 나란히 놓기 전까지 아무도 몰랐습니다. 이름을 맞추고, 마크다운 본문이 별도 경로로 시스템 폰트를 쓰던 것도 함께 고쳤습니다.
