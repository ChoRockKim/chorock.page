...
- **`-` 리스트의 첫 항목이 도입 문장과 너무 멀던 문제.** 실측하니 도입 문장→첫 항목이
  **30px**(문단 하단 여백이 ul 기본 margin-top 16px을 상쇄로 이긴다)인데 항목 사이는 10px,
  리스트 끝→다음 문단은 16px — **리스트가 자기 도입 문장보다 다음 문단에 붙어 보이는
  역전**이었다. 도입 문장(`p:has(+ ul/ol)`)의 하단과 리스트 상단을 둘 다 10px로 내려 항목
  간격과 맞췄다. 제목 바로 뒤 리스트도 같은 방향으로 16→10px.
  - 중첩 리스트(li 안의 ul/ol)는 UA 기본이 margin-top 0이라 바깥 규칙이 오히려 10px을
    새로 벌려놓게 되므로 `li ul/ol { margin-top: 0 }`으로 되돌렸다.
  - 리스트 끝→다음 문단(16px)과 항목 간격(10px)은 실측으로 불변 확인.

## [0.11.9] - 2026-09-14

### Fixed

- **글 상세에서 세로로 연속한 이미지 사이 간격이 너무 넓던 문제.** 에디터에서 이미지를
  연달아 붙이는 방법이 둘인데 둘 다 넓었다(로컬 프로덕션 빌드에 실제 구조를 주입해 실측):
  - 빈 줄로 구분(이미지 단독 문단 연속) → 문단 하단 여백이 그대로 남아 **30px**.
  - 한 줄 개행(같은 문단 안 `img<br>img`) → remark-breaks가 넣은 `<br>`이 블록 이미지
    사이에 빈 줄박스(≈29px)를 만들고 이미지 위아래 margin과 합쳐 **≈69px**.
  - 둘 다 **10px**로 좁혔다. ②의 `<br>`은 `display: none` — 이미지가 `display: block`이라
    줄바꿈 역할이 없고 빈 줄만 만든다. 인접 형제 결합자가 텍스트 노드를 건너뛰므로
    `br:has(+ img)`가 사이의 개행 텍스트와 무관하게 다음 이미지를 본다.
  - 건드리지 않은 것도 실측으로 확인: 이미지 뒤에 텍스트가 이어지는 `<br>`은 그대로이고
    (이미지→텍스트 간격 불변), 같은 줄에 쓴 2-up 가로 배치(`p:has(img + img)`)는 `<br>`이
    끼면 DOM 기준으로 애초에 매칭되지 않아 세로 쌓임이 유지된다. gap 15px도 그대로.
  - 현재 공개된 19개 글에는 직접 연속한 이미지가 한 건도 없어(전수 스캔) 기존 글 외관은
    변하지 않는다 — 앞으로 쓸 글과 에디터 미리보기(`.pd-body`를 공유)를 위한 수정이다.

## [0.11.8] - 2026-09-14

### Changed

- **forA 프로젝트 소개를 worklog 9/9~9/14 기준으로 갱신했다**
  (`scripts/seed-projects.ts` — 이 파일이 Project 컬렉션의 단일 출처이므로
  `npm run seed:projects`가 반영 경로다).
  - 사실 갱신: "출시 가능한 상태까지" → **1.6.0으로 양대 스토어 실제 출시**(2026.09, 3개월
    반 만의 네이티브 릴리즈). 주요 기능에 30일 재연결·계정 병합·소셜 계정 관리 반영.
  - 새 이야기 3건(사용자 선택): 설계 판단에 "릴리즈 전제를 문서가 아니라 실측으로"(프로파일
    직접 복호화, EAS 환경변수의 네이버 키 누락 — 조용한 실패 경로, 2FA 우회 판단), 문제
    해결에 "원인을 두 번 잘못 짚었다"(삼중 오진 — 닫힌 고리, 오진을 지우지 않는 이유)와
    "App Store 자동 분석 반려"(config plugin이 조용히 넣은 권한 문구 3건).
  - 분량은 교체·압축: 매거진 기능 3항목 → 2항목, "글자 잘림"과 "폰트 폴백"을 "에러 없이
    조용히 틀어져 있던 렌더링 둘"로 병합, 기존 문제 해결 항목들 압축. 데드락 2건(닉네임
    잠금, 병합 순환)은 넣지 않기로 했다.
  - 프로젝트 상세 목차는 h2만 필터하므로(`depth === 2`) h3가 늘어나도 영향 없음을 확인.
  - forA를 목록 맨 위로 올렸다. 목록은 `publishedAt` 내림차순이고 화면에 날짜가 표시되지
    않으므로, forA의 `publishedAt`을 전체에서 가장 늦은 2026-09-14(1.6.0 스토어 출시일)로
    바꾸는 것이 정렬 수단이다.
...
- **스크롤되는 목차에서 활성 항목이 상자 밖으로 밀려나면 목차만 따라 굴러간다.** 목차가 길어
  스크롤이 생긴 글에서는 읽는 위치가 상자 아래로 넘어가 버려, 정작 "내가 어디쯤인지"를
  알려주는 표시를 못 보게 된다는 제보. 실측으로도 확인됐다 — 목차가 가장 긴 글에서 페이지를
  500px씩 내리며 17번 확인했더니 **6번은 활성 항목이 목차 상자 밖**에 있었다.
  - **`scrollIntoView`는 쓸 수 없다.** 그건 스크롤 가능한 *모든 조상*을 움직이므로 문서까지
    같이 스크롤돼, 읽는 중인 페이지가 제멋대로 튄다. 컨테이너의 `scrollBy`만 부르면 그 상자
    하나만 움직인다. 검증에서 페이지 스크롤 위치가 한 번도 흔들리지 않는지를 같이 본다.
  - 스크롤 컨테이너가 화면마다 다르다. `/posts/[slug]`에서는 `<aside>` 자신이고
    (`.pd-grid .toc-desktop`), `/projects/[slug]`에서는 조상인 `.proj-toc`다. 자신부터 위로
    올라가며 실제로 넘치는 첫 요소를 찾되 **`<body>`에 닿기 전에 멈춘다** — 문서 자체를
    컨테이너로 잡으면 위에 적은 "페이지가 튀는" 바로 그 동작이 된다.
  - 항목을 가장자리에 딱 붙이지 않고 28px쯤 여유를 남긴다(위아래로 더 있다는 게 보인다).
    다만 **그 여유는 남는 공간의 절반으로 묶어야 한다** — 프로젝트 사이드바는 낮은 화면에서
    상자가 90px까지 줄어드는데, 그러면 위아래 여유를 합친 값이 항목 하나 자리보다 커져 위로
    밀어도 아래로 밀어도 조건이 계속 참인 상태가 된다. 실제로 그 화면에서 항목이 끝내 다
    안 보이는 걸 확인하고 고쳤다.
  - 첫 적용은 즉시(`behavior: "auto"`), 그 뒤로는 부드럽게. 글 중간에서 새로고침한 경우
    맨 위에서부터 굴러 내려오는 게 오히려 산만하다. `prefers-reduced-motion`은 CSS 애니메이션이
    아니라 JS 스크롤이므로 미디어 쿼리가 아니라 `matchMedia`로 읽는다(`Header`의 테마 토글과
    같은 이유).
  - 검증: 뷰포트·감축모션·화면 종류를 바꿔 5가지 조합, 표본 77개 — **가려짐 0회, 페이지가
    끌려간 경우 0회, 페이지 에러 0회.** 같은 스크립트를 수정 전 프로덕션에 돌리면 6회 가려진다.

## [0.11.6] - 2026-09-10

### Changed

- **목차 스크롤바를 평소에는 숨기고, 목차에 마우스를 올렸을 때만 얇은 초록 막대로 보여준다.**
  목차가 길면 `.pd-grid .toc-desktop`에 `overflow-y: auto`가 걸리는데, 그때 나오는 브라우저
  기본 스크롤바가 200px짜리 목차 열에 비해 너무 굵고 항목을 가린다는 제보.
  - 가리는 이유가 폭 때문이 아니다. 실측해 보면 `offsetWidth - clientWidth === 0`인데도 바가
    보인다 — 크로뮴/맥의 **오버레이 스크롤바**라 레이아웃을 전혀 차지하지 않는 대신 글자
    **위에 겹쳐** 그려진다. 그래서 열을 넓히는 식으로는 해결되지 않는다.
    (목차가 가장 긴 글은 항목 21개로 `clientHeight 573 / scrollHeight 811`, 실제로 스크롤된다.)
  - 기본은 `scrollbar-color: transparent transparent`, `:hover`/`:focus-within`에서만
    `color-mix(in srgb, var(--color-accent) 70%, transparent)`. `--color-accent`가 이미
    테마별로 정의돼 있어 라이트(`#4caf50`)·다크(`#5aa860`)가 자동으로 맞는다.
  - **`scrollbar-width`는 상태와 무관하게 `thin`으로 고정한다.** 클래식 스크롤바를 쓰는
    환경(윈도·리눅스)에서는 폭이 레이아웃을 차지하므로, 호버에서 폭까지 바꾸면 마우스를
    올릴 때마다 목차가 좌우로 움찔한다. 바뀌는 건 색뿐이다.
  - 표준 속성과 `::-webkit-scrollbar` 의사요소를 **같이** 둔다. 충돌이 아니다 —
    Blink/Gecko는 `scrollbar-color`/`scrollbar-width`가 선언된 순간 의사요소 규칙을 무시하고,
    그 속성을 모르는 사파리 18.2 미만만 의사요소 쪽을 쓴다. 엔진마다 정확히 하나씩 적용된다.
  - 프로젝트 상세의 `.proj-toc`에도 같이 건다 — 같은 `TableOfContents` 컴포넌트이고 801px
    이상에서 똑같이 스크롤된다. 한쪽만 고치면 두 화면이 엇갈린다.
  - 검증은 계산값으로 했다(1440×700, 다크·라이트, 글 상세·프로젝트 상세 3종 전부): 기본
    `transparent transparent` → 호버 시 테마별 초록, 그리고 **호버 전후 `offsetWidth -
    clientWidth`가 그대로**(레이아웃 안 움찔). 다만 이 환경의 헤드리스 크로뮴은 오버레이
    스크롤바를 스크린샷에 아예 안 그려서(엘리먼트·클리핑 스크린샷 둘 다 확인) "실제로
    보기 좋은지"는 실제 브라우저에서 눈으로 봐야 한다.

## [0.11.5] - 2026-09-10

### Fixed

- **스크롤한 상태에서 하드 리프레시하면 헤더 캡슐의 내비 항목이 세로로 접히던 문제.**
  `소/개`, `시리/즈`, `프로젝/트`처럼 항목마다 한 글자씩 잘려 두 줄이 되고, 한 번 그렇게
  되면 리사이즈 전까지 그대로 굳었다.
  - 원인은 폰트도 CSS도 아니고 **측정이다.** `Header.tsx`의 `measure()`는 캡슐 폭
    `--bar-w`를 자식들의 `getBoundingClientRect().width` 합으로 구하는데, 이 값은
    **조상에 걸린 transform이 반영된** 값이다. 캡슐이 접힐 때 `globals.css`의
    `dropletIn`(0.85s)이 `.site-bar`에 `scale()`을 걸고, scaleX는 타임라인 15% 지점에서
    **0.98551**까지 내려간다. 그 사이에 측정이 돌면 합계가 그만큼 작게 나오고 그 값이
    굳는다. `.site-bar`는 `flex-wrap: nowrap`이라 바 자체는 안 접히는 대신 자식이
    눌리고, 눌린 내비 링크의 **글자가** 줄바꿈된다 — 한글은 아무 글자에서나 끊기므로
    **1px만 모자라도** 항목마다 한 글자씩 밀린다.
  - 두 조건이 동시에 맞아야 터진다. **스크롤**이 있어야 `is-condensed`가 붙어
    애니메이션이 돌고 `width: var(--bar-w)`가 적용되며, **하드 리프레시**여야 폰트를
    네트워크에서 받아 `document.fonts.ready.then(measure)`가 애니메이션 재생 구간 안에서
    늦게 풀린다. 캐시된 새로고침에서는 마운트 직후(scale=1)에 풀려 정상값이 나온다.
    그 뒤로는 `resize` 때만 다시 재므로 한 번 작게 굳으면 계속 그대로다.
  - 해결: 측정할 때 `.site-bar`의 현재 배율(`DOMMatrixReadOnly`의 `a`/`d`)로 되나눠
    변형과 무관한 폭·높이를 얻는다. 펼쳐진 상태의 computed transform은 `"none"`이고
    `DOMMatrixReadOnly("none")`은 던지므로 가드가 필요하다. 렌즈 변위 맵 크기로 쓰이는
    `barH`도 같은 이유로 오염됐다(scaleY는 +4.5%까지 올라간다).
  - 안전장치로 내비 링크에 `white-space: nowrap`을 준다. 앞으로 어떤 이유로든 폭이
    모자라도 글자가 쌓이는 대신 몇 px 넘칠 뿐이다.
  - **`offsetWidth`는 대안이 아니다.** transform을 무시하는 건 맞지만 정수로 반올림돼
    여기서는 합계가 **항상 1px 모자라고**, 그러면 항상 접힌다.
  - 계측(Playwright Chromium, 정답은 462px): 애니메이션이 도는 동안 현재 방식의 합계는
    **456~464px**로 출렁이는데, 배율로 정규화하면 **462px로 고정**된다.
    Pretendard CSS 요청을 0~1200ms까지 지연시키며 스크롤된 채 새로고침을 반복하는
    회귀 스크립트로 확인했다 — 수정 전에는 특정 지연에서 `--bar-w: 458~459px`와 세 항목
    접힘이 재현되고, 수정 후에는 11회 전부 `--bar-w`가 애니메이션 내내 462에서 벗어나지
    않는다. 캡슐의 폭·높이·위치·모서리·transform은 수정 전후가 동일하다(보이는 동작은
    그대로고 측정 방식만 바뀐다).
...
- **iOS에서 인라인 `코드` 칩이 바로 앞 글자를 덮어쓰던 문제.** 아이폰 크롬 제보 —
  `접속 요청 <---> 화면에 렌더링 인 `FCP` 가 굉장히 단축된다` 문장에서 `FCP` 칩이 왼쪽으로
  밀려 "인" 글자의 절반을 가렸다. 원인은 칩이 아니라 **같은 줄 앞쪽에 있던 `<--->`**다.
  Pretendard에는 이 문자열을 하나의 긴 화살표(⟵)로 합치는 합자(ligature)가 있는데, WebKit은
  그 합자를 **그릴 때만** 쓰고 줄 배치에는 합자 이전 낱글자 폭 합계를 쓴다. 그래서 그 텍스트
  노드는 실제로 칠해지는 폭보다 좁게 자리를 잡고, 같은 줄에서 뒤따르는 인라인 박스가 그만큼
  왼쪽으로 당겨져 앞 글자 위에 겹친다. iOS는 브라우저가 전부 WebKit이라 사파리·크롬 모두 해당된다.
  - 해결: `app/globals.css`의 `body`에 `font-variant-ligatures: none`. `font-feature-settings:
    "liga" 0`이나 `no-contextual` **하나만으로는 이 화살표가 죽지 않는다**(둘을 같이 꺼야 한다).
    그리고 `font-variant-ligatures`의 계산값이 `font-feature-settings`를 이기므로, 저수준 속성만
    건드리는 방식은 애초에 무시된다. 사이트 전역으로 건 이유는 이 어긋남이 "합자 + 바로 뒤
    인라인 요소"라는 조합이면 어디서든(카드 요약, 제목, 링크 앞뒤) 똑같이 터지기 때문이다.
    부작용은 `<--->`가 화살표 그림 대신 친 그대로 나오는 것 — 마크다운에 그렇게 친 사람 기대와
    오히려 맞는다.
  - **눈이 아니라 계측으로 잡았다.** iOS를 이 환경에서 띄울 수 없으니 Playwright WebKit 26으로
    같은 페이지(402px 뷰포트)를 열어 재현했다: 화살표 5글자를 그릴 때는 51.6px를 쓰면서 배치는
    38.6px만 잡아, `FCP` 칩이 Chromium 대비 정확히 **13.03px 왼쪽**(257.72 → 244.69)에 놓였다.
    제보 스크린샷의 픽셀을 직접 재서 나온 값(244.7)과 일치한다 — 즉 Playwright WebKit이 iOS와
    같은 현상을 그대로 낸다. 범인 특정도 DOM을 바꿔가며 이분 탐색했다: `<--->`를 `ABCDE`나
    `가나다라마`로 바꾸면 두 엔진의 좌표가 즉시 같아졌고, 칩 쪽 스타일(패딩·글꼴 크기·배경·
    `display: inline-block`·`opacity`)은 **무엇을 꺼도 좌표가 1px도 안 움직였다**.
  - 같은 텍스트 노드 *안*의 글자들은 위치가 멀쩡하다(화살표 뒤의 "화면에"는 Chromium과 동일
    좌표). 그래서 "가끔만 깨진다"처럼 보였던 것이고, 합자 뒤에 인라인 요소가 오는 줄에서만
    재현된다.
...
- **조회수·방문자 수가 0에서 실제 값까지 세어 올라간다.** fetch 응답이 늦다 보니 숫자가 불쑥
  나타나거나(목록: 불러오기 전엔 아무것도 안 그렸다) 한 프레임에 툭 바뀌는 게(상세·방문자 수)
  거슬린다는 피드백. 셋 다 0을 먼저 보여주고 도착하면 굴러 올라간다.
  - 공통 훅 `components/useCountUp.ts`. 감속 이징(ease-out) — 등속은 기계적으로 보이고 끝에서
    느려져야 "맞춰지는" 느낌이 난다. 지속 시간은 크기에 맞춰 300~900ms로 조절한다(0→2를 700ms
    끄는 건 굼뜨고, 0→155를 200ms에 끝내면 그냥 점프로 보인다).
  - 목록 카드가 이제 로딩 중에도 `조회 0`을 그린다 — "갑자기 나타남"의 원인이던 부분이다.
    0.11.1에서 카드 여러 장이 동시에 0에서 튀는 걸 피하려던 선택인데, 세어 올리면 그 전환이
    오히려 볼 만해진다.
  - `/about`의 방문자 수(today·total)도 같은 훅을 쓴다.
  - 숫자에 `.tnum`(`font-variant-numeric: tabular-nums`)을 준다. 없으면 매 프레임 자릿수 폭이
    달라 주변 텍스트가 좌우로 떨린다.

### Fixed

- **애니메이션이 최종값에 도달하는 유일한 경로면 안 된다.** 첫 구현은 rAF 콜백 안에서만 실제
  숫자를 넣었는데, rAF는 배경 탭에서 멈추고 부하가 크면 아예 굶는다 — 이 저장소의 개발 머신
  Chrome이 실제로 그렇다. 조회수 60인 글이 **"조회 0"에 갇힌** 채 400ms 동안 rAF가 한 번도
  돌지 않는 것을 확인했다(fetch 자체는 46ms에 끝났다). 애니메이션이 빠지는 게 아니라 **틀린
  숫자가 화면에 남는** 문제다. `setTimeout(duration + 400)` 안전장치로 애니메이션이 못 끝나면
  값을 맞춘다 — setTimeout도 배경 탭에서 억제되지만 실행은 되므로 언젠가 반드시 안착한다.

  검증은 rAF 굶주림을 우회해서 했다. iframe의 `requestAnimationFrame`을 `setTimeout(…, 16)`
  shim으로 갈아끼우고 `MutationObserver`로 값 변화를 기록하니(루프 폴링은 이 머신 렌더러를
  멈추게 한다) 실제 단계열이 나왔다 — `0→4→8→…→61`, 32단계, 증분이 4→1로 감소(감속 이징이
  눈이 아니라 데이터로 확인된다). 같은 iframe에서 `matchMedia`를 패치해 감축 모션 분기도
  확인했다(0→61, 중간값 없음). JS 애니메이션이라 이 선호는 CSS 미디어 쿼리가 아니라
  `matchMedia`로 읽어야 한다.
...
- **글 목록 카드에도 조회수를 표시한다** — `/posts`, `/posts/page/[n]`, `/about`의 최근 글,
  글 상세 하단의 관련 글까지(전부 `PostCard`를 쓴다).
  - **조회수를 목록 데이터에 싣지 않았다.** `/posts`는 `getCachedPosts`(`unstable_cache`, 300초)를
    거치는데 조회수는 계속 변하므로 캐시에 넣으면 항상 묵은 숫자가 된다. 캐시는 그대로 두고 숫자만
    클라이언트에서 따로 가져온다(상세 페이지의 `PostViewCounter`와 같은 이유).
  - **카드마다 요청하지 않는다.** `components/PostViews.tsx`가 같은 틱에 마운트된 카드들의 slug를
    모아 `GET /api/post-views?slugs=...` **한 요청**으로 묻는다. 카드가 5장이든 3장이든 1건이다
    (브라우저에서 실제로 세어 확인). 모듈 수준 배치라 `PostCard`가 서버·클라이언트 양쪽에서
    렌더되는데도 provider가 필요 없다.
  - **목록은 조회수를 올리지 않는다.** 집계는 상세 페이지에서만 일어난다 — 목록에 뜬 것만으로 오르면
    숫자가 의미를 잃는다. 페이지를 오가며 여러 번 렌더해도 값이 그대로인 것을 확인했다.
  - 목록에서는 불러오기 전에 아무것도 그리지 않는다. 카드가 여러 장이라 전부 "조회 0"을 띄웠다가
    한꺼번에 바뀌면 그 자체가 깜빡임으로 읽힌다(카드가 하나뿐인 상세 페이지는 0을 먼저 그리는
    기존 방식을 유지).
  - API `GET`은 `?slugs=a,b` 배치를 받는다. 기록이 없는 slug도 0으로 채워 돌려주고(클라이언트가
    빈 값을 따로 다루지 않아도 되게), 상한 50개를 넘으면 400이다. 기존 `?slug=` 단일 조회는 그대로.

## [0.11.0] - 2026-09-07

### Added

- **글 상세에 조회수를 표시한다.** 메타 줄이 `날짜 · N분 읽기 · 조회 N`이 된다.
  - **클라이언트에서 세고 클라이언트에서 그린다.** `/posts/[slug]`는 ISR(`revalidate = 300`)이라
    서버에서 렌더하면 최대 5분 묵은 숫자가 정적 HTML에 박힌다 — `/about`의 방문자 카운터가
    클라이언트 컴포넌트인 것과 같은 이유다. `/posts/[slug]`가 `●`(SSG)로 유지되는 것도 확인했다.
  - **기록과 조회를 한 요청으로 합쳤다.** `POST /api/post-views`가 1 올리고 그 결과 총합까지
    돌려준다. `/about` 카운터는 기록(POST)과 표시(GET)가 별개 컴포넌트라 첫 방문에 순서 보장이
    없어 "오늘 1 · 총 0"이 잠깐 보이는 알려진 quirk가 있는데, 여기서는 그 경합 자체가 없다.
  - **중복 방지는 브라우저 localStorage로 24시간.** 같은 글을 다시 열면 올리지 않고 읽기만 한다
    (`GET`). 방문자 카운터처럼 httpOnly 쿠키로 막으려면 "본 글 목록"을 쿠키 하나에 담아야 하는데
    글이 늘수록 4KB 제한에 걸리고 만료 관리가 붙는다 — 장식성 지표라 그만한 값을 치를 이유가 없다.
    localStorage가 막힌 환경(사파리 프라이빗 등)에서는 중복 방지만 포기하고 집계는 계속한다.
  - **저장은 `models/PostView.ts`의 `{ slug, date, count }`**, `(slug, date)` 복합 유니크 인덱스.
    `DailyVisit`과 같은 모양이라 총합은 SUM 집계로 구한다 — 별도 누적 카운터가 없으니 두 숫자가
    어긋날 수 없다. 유니크가 아니면 동시 요청이 같은 날짜에 문서를 여러 개 만든다. 날짜별로 쪼갠
    덕에 나중에 기간별 통계를 새로 만들지 않고도 낼 수 있다.
  - 기존 글은 전부 0에서 시작한다(과거 기록이 없다).

## [0.10.9] - 2026-09-07

### Fixed

- **프로젝트 상세에서 검색을 열면 왼쪽 사이드바가 통째로 사라지던 문제.** 0.9.0에서 넣은 규칙이
  `.toc-desktop`과 `.proj-sidebar`를 **폭 조건 없이** 비우고 있었다. 글 상세의 목차는 모달과 같은
  자리(오른쪽 위)라 실제로 글자가 겹쳐 읽히지만, 프로젝트 사이드바는 **왼쪽 열**이라 오른쪽 정렬
  모달과 애초에 겹치지 않는다. 실측: 1440px에서 모달 948~1410 vs 사이드바 270~530, 2열이 유지되는
  최저폭 856px에서도 74px이 남는다. 그런데도 제목·설명·프로젝트 정보·버튼·목차가 전부 사라졌다.
  `.proj-grid`가 1열로 무너지는 800px 이하에서만(386px에서 모달 12~374 vs 사이드바 30~356으로
  실제로 겹친다) 비우도록 좁혔고, 목차를 비우는 규칙도 `.pd-grid` 안으로 한정했다 — 그러지 않으면
  프로젝트 사이드바 **안의** 목차가 같은 선택자에 계속 걸린다.
- **그 과정에서 사이드바 페이드가 죽어 있던 것도 찾아 고쳤다.** `.proj-sidebar`에 `transition`
  선언이 두 곳에 있었는데, `transition`은 단축 속성이라 뒤에 오는 선언이 앞의 것을 **통째로**
  대체한다. 0.10.3에서 넣은 `transition: top 0.4s`가 검색 모달용 `transition: opacity 0.2s`를
  밀어내 좁은 화면에서 페이드 없이 툭 사라졌다 나타났다. 둘을 한 선언으로 합쳤다.
  0.10.6의 `.elev-sm`이 `box-shadow`를 덮은 것과 같은 종류의 함정이다.

## [0.10.8] - 2026-09-06

### Fixed

- **글 상세의 목차가 헤더 캡슐 바로 밑에 붙어 답답하던 문제.** 접힌 캡슐 아래가 77px, 목차 위가
  80px이라 **3px**밖에 없었다. 캡슐(x 948~1410)과 목차(x 960~1160)가 가로로도 겹쳐 있어 얹힌
  것처럼 보였다. 30px을 띄웠고, 오프셋은 하드코딩 `80`이 아니라 헤더가 실측해 `:root`에 심는
  `--header-h` 기준(`calc(var(--header-h) + var(--space-8))`)으로 바꿨다.
  `/projects/[slug]`는 `.proj-sidebar`가 이 값을 `position: static !important`로 덮으므로
  영향이 없다(계산값으로 확인).
- **그 여파로 긴 목차가 화면 밖으로 나갈 뻔한 것을 함께 막았다.** 27px을 내리자 항목이 가장 많은
  글(19개·634px)이 748px 뷰포트에서 아래 여유 **7px**까지 줄었다. `position: sticky` 요소가
  뷰포트보다 길면 잘리는 게 아니라 **아랫부분에 영영 닿을 수 없다** — 스크롤은 페이지를 움직이고
  고정된 상자는 그대로다(`.proj-sidebar`에서 이미 겪은 함정). `.pd-grid .toc-desktop`에만
  최대 높이와 내부 스크롤을 줬다. 짧은 목차에는 스크롤바가 생기지 않고(넘칠 때만 동작),
  스크롤스파이 표시자도 내부 스크롤 후 활성 항목과 정확히 같은 줄을 유지한다(차이 0px).
...
- **프로젝트 상세에서 「맡은 일」·「성과와 한계」를 개요 도입 문단 바로 아래로 올리고 카드로
  강조했다.** 채용 담당자가 가장 먼저 볼 두 항목인데 성과가 본문 맨 끝(스크린샷 직전)에 묻혀
  있었다. 7개 프로젝트 전부에 적용했다.
  - **원문 순서도 함께 옮겼다.** 목차는 `extractHeadings(project.overviewMd)`가 마크다운 원문
    순서로 만들기 때문에, 화면에서만 순서를 바꾸면 목차와 본문이 어긋난다. 시드에서 섹션 블록을
    통째로 이동했고(글자는 한 자도 수정하지 않았다 — 정렬한 줄 집합과 글자 수가 이동 전후 동일),
    그러니 목차·본문·스크롤스파이가 저절로 일치한다.
  - **마크다운은 여전히 한 번만 컴파일한다.** 섹션별로 `compileMarkdown`을 나눠 부르면 호출마다
    `GithubSlugger`가 새로 생겨 중복 제목의 id 접미사가 달라지고 목차 앵커가 조용히 깨진다
    (`RevealBlocks`가 섹션을 재컴파일하지 않는 것과 같은 이유). `components/ProjectOverview.tsx`는
    컴파일 결과의 **자식 배열을 잘라 감싸기만** 한다 — 엘리먼트를 다시 만들지 않으므로 id가 살아
    있다. 카드 대상은 제목 텍스트가 아니라 `extractHeadings`가 계산한 id로 찾는다.
  - 카드가 `.pd-body`의 직접 자식이 되므로 카드 한 덩어리가 한 번에 페이드된다(블록마다 따로
    페이드되던 기존 동작보다 카드에는 이쪽이 맞다). 카드 안 `p`/`ul`은 `.pd-body p`의 자손
    선택자를 그대로 받아 0.88 농도로 읽힌다 — 추가 규칙이 필요 없음을 계산값으로 확인했다.
  - `.pd-body h2`의 60px 위 여백은 카드 안에서 제목을 밀어내므로 `margin-top: 0`으로 덮되,
    `scroll-margin-top: 90px`은 건드리지 않았다(목차 앵커 점프가 그 값에 의존한다).

- **forA의 누적 지표를 갱신했다** — 다운로드 1,450건 → 2,000건 이상, 가입 회원 1,000명 →
  1,500명 이상. DB에는 앞서 시드로 반영했고, 저장소가 뒤늦게 따라붙는 것을 막기 위해 이번
  커밋에 함께 담는다(시드 스크립트가 이 수치의 유일한 출처다).

### Fixed

- **시리즈 카드의 왼쪽 액센트 괘선이 0.10.2 배포 이후 한 번도 렌더되지 않고 있었다.** 원인이 두
  겹이었고, 카드 표면이 배경과 1.1:1밖에 차이가 안 나 눈으로는 티가 나지 않았다 — 계산된
  `box-shadow`를 읽고서야 드러났다(값이 `.elev-sm`의 그림자 하나뿐이었다).
  - **존재하지 않는 `--color-border`를 썼다**(올바른 이름은 `--color-divider`). 정의되지 않은
    `var()`는 그 선언 **전체**를 무효로 만들기 때문에 괘선까지 같이 사라진다.
  - 그걸 고쳐도 마크업의 `.elev-sm`이 `.series-card`보다 CSS에서 **뒤에** 있어 같은 특이도로
    `box-shadow`를 통째로 덮었다. 카드에서 `elev-sm`을 뺐다 — `.series-card`의 마지막 레이어인
    `var(--shadow-sm)`가 같은 역할을 한다.

## [0.10.5] - 2026-09-05

### Changed

- **`/about` 히어로의 하이라이트를 세 줄에서 두 줄로 줄이고, 라벨을 윗줄 키커로 올렸다.**
  잘 안 읽힌다는 지적을 받아 원인을 셋으로 나눴다.
  - **줄마다 대시가 두 개씩 찍혔다.** 렌더 구조가 `— **라벨** — 설명`이라 세 줄에 여섯 개였다.
    앞머리 대시와 구분자 대시를 둘 다 없앴다(렌더된 HTML에서 0개 확인).
  - **라벨의 문법 층위가 제각각이었다.** `실서비스 운영`(명사구) / `혼자 끝까지`(부사구) /
    `검증을 남깁니다`(완결 서술문). 눈이 먼저 비교하는 게 라벨인데 셋이 다른 모양이었고, 설명이
    줄바꿈되면 라벨 열이 무너져 스캔 목록이 아니라 들쭉날쭉한 산문으로 읽혔다. 라벨에 자기 줄을
    주면 줄바꿈이 정렬을 깨지 못한다. 남은 둘은 명사구로 통일하고 숫자를 라벨 자리로 끌어올렸다
    (`혼자 끝까지` → `사용자 140명`) — 스캔하는 사람이 라벨만 읽어도 사실을 얻는다.
  - **세 번째 줄이 바로 위 `intro`의 마지막 문장과 거의 같은 말이었다.** 의도된 반복이라 주석에
    적혀 있었지만 세 줄 아래에 붙어 있으니 강조가 아니라 중복으로 읽혔고, 구체적 사실이 없는
    유일한 줄이기도 했다. 삭제했다 — **검증 메시지는 이제 `intro`에만 있다.** `intro` 자체는
    건드리지 않았다.

### Fixed

- **라벨 색이 라이트 모드에서 읽히지 않던 문제.** 키커에 `--color-accent`를 쓰면 라이트에서
  `#4caf50` on `#f3f2f2` = **2.49:1**로, 11px 텍스트 기준 4.5:1에 한참 못 미친다. 라벨을 스캔
  기준점으로 승격시켜 놓고 가장 안 읽히게 두면 변경 자체가 무의미해진다. 테마별로 값이 뒤집히는
  `--color-accent-800`(`#2e7d32` 라이트 / `#a8dfae` 다크)으로 바꿔 4.59:1 · 11.41:1을 확보했다.
  `.tag-accent`가 이미 쓰던 토큰이라 새 규칙은 아니다.

## [0.10.4] - 2026-09-05

### Fixed

- **스킬/기술 태그의 아이콘 전수조사.** SkillTag로 들어가는 이름을 전부 모아(/about의 SKILLS·경력
  태그 + Mongo에 있는 프로젝트 7개의 `tags`와 `stack[].items`, 총 70개) 대조했더니 **41개가 아이콘
  없이 렌더**되고 있었다. 원인은 두 갈래였고 하나만 고칠 수 있는 문제였다.
  - **이름 변형(대부분).** 맵이 정확히 일치하는 이름만 찾는데 실제 데이터에는 `React 19`,
    `Expo (SDK 56)`, `React Native 0.86`, `Next.js (App Router)`, `Supabase (Postgres·Storage)`
    처럼 부연이 붙어 있었다. 별칭을 하나씩 늘리는 대신 반복되는 두 패턴 — **끝에 붙는 괄호**와
    **끝에 붙는 버전 표기** — 을 `normalizeSkillName()`에서 떼어낸다. 나중에 `React 20`이라고 써도
    자동으로 잡힌다. `html5`/`css3`처럼 공백 없이 숫자가 붙은 이름은 영향이 없다(버전 규칙이 앞
    공백을 요구한다).
  - Expo 생태계 패키지 6개·React Native 계열 3개의 별칭과, 실재를 확인한 신규 슬러그
    `mongoose`/`swiper`/`formspree`/`remark`/`testinglibrary`를 추가했다. **아이콘 29개 → 53개.**
  - 남은 17개는 Simple Icons에 아이콘이 없어 고칠 수 없다(전부 직접 404 확인). AWS 계열은 이미
    알려져 있었고, **`OpenAI`/`ChatGPT`도 같은 방식으로 사라졌다는 것을 이번에 확인했다.** 없는
    슬러그를 넣어봐야 `<img>`가 404를 받고 `onError`로 사라질 뿐이다.
  - 기존에 등록된 슬러그 52개는 전부 아직 살아 있다(죽은 슬러그 0건).

### Changed

- **다크모드에서 아이콘이 묻히는지 판단하는 기준을 눈대중에서 계산값으로 바꿨다.**
  `MONOCHROME_ICON_SLUGS` → `DARK_RECOLOR_ICON_SLUGS`(8개 → 16개). 기준은 "거의 검정"이 아니라
  **다크 배경 `#1b1a1d` 대비 3:1 미만**이고, 각 아이콘의 브랜드 hex를 Simple Icons에서 받아
  계산했다. 옛 기준이 놓치고 있던 `axios` #5A29E4(2.37) · `jest` #C21325(2.82)가 대표적이다 —
  검정이 아니라 그냥 어두운 유채색이라 눈대중으로는 걸러지지 않았다.
- **재색칠은 다크모드에서만 한다.** 예전에는 테마와 무관하게 `--color-text`로 덮었는데, 목록을
  늘리면 라이트에서는 멀쩡한 Axios 보라·Flutter 파랑까지 검게 변한다. 라이트에서는 브랜드색을
  그대로 둔다.
...
- **프로젝트 상세의 왼쪽 사이드바가 너무 아래에 붙어 목차 아래쪽이 잘리던 문제.** 748px 뷰포트에서
  목차가 쓸 수 있는 높이는 148px인데 내용이 208px이라 60px이 잘려 있었다. 사이드바 위쪽 정보가
  세로를 다 먹고, 줄어들 수 있는 항목이 목차뿐이라(`min-height: 0`이 거기에만 있다) 전부 목차가
  떠안는 구조였다.
  - **헤더가 캡슐로 접히면 화면 왼쪽 위가 통째로 빈다**(캡슐은 오른쪽 정렬 ~462px). 그때 사이드바를
    `top: 20px`까지 끌어올리고 `max-height`도 같이 늘려 57px을 목차에 준다. 결과적으로 목차가
    208px 전부를 확보해 **잘림이 사라졌다**. 1440px에서 캡슐과의 간격은 418px, 이 레이아웃이
    적용되는 최저폭(806px)에서도 24px 남아 겹치지 않는다.
  - `is-condensed`는 `<header>`에만 붙어 있어 헤더 바깥에서 읽을 수 없다. `search-open`과 같은
    방식으로 `<html>`에도 `header-condensed`를 심었다.
  - 평소 오프셋도 하드코딩 `80px` 대신 헤더가 실측해 심는 `--header-h` 기준으로 바꿨다.
  - `top`에 0.4s 전이를 넣었다. 없으면 접히는 순간 이미 고정된 사이드바가 한 프레임에 튄다.

## [0.10.2] - 2026-09-05

### Changed

- **시리즈 목록 카드의 모서리를 둥글렸다**(`--radius-card` 20px). 각진 편집 시스템의 예외지만
  헤더 유리·`.chip-btn`이 이미 만들어 둔 예외라 새 규칙은 아니다. 왼쪽 액센트 괘선은
  `border-left`에서 **안쪽 그림자로 바꿨다** — `border-left`는 직각이라 둥근 모서리와 부딪히는데,
  `inset` 그림자는 반경을 따라 잘린다. 호버는 색만 바뀌던 것에서 2px 떠오르는 것으로 바꿨다.
- **`/about`의 스킬 태그를 알약 모양 + 옅은 액센트 채움으로 바꿨다**(`.tag-skill`).
  `.tag-outline`을 고치지 않고 새 클래스를 뺀 이유는 그 클래스를 `/posts`의 필터 칩과
  페이지네이션 버튼도 쓰기 때문이다 — 거기까지 같이 바뀐다.

### 폐기한 시안 (기록용)

같은 카드를 두 번 다르게 만들었다가 둘 다 물렸다. 다시 시도하지 않기 위해 남긴다.

- **배경 오로라 + 유리 카드.** `backdrop-filter`는 뒤에 있는 것을 흐리는 재질이라 단색 페이지
  위에서는 "살짝 틴트된 상자"밖에 되지 않는다. 그래서 액센트 블롭 배경을 깔아야 했고, 그렇게까지
  하고도 결과는 탁한 상자였다.
- **slug 해시로 만든 그라디언트 썸네일.** 결정론·대비·모바일 배치까지 전부 검증했지만
  (시리즈 4개가 서로 다른 색, 흰 글자 대비 4.5~8.1:1), 목록이 시끄러워졌다. 정보량이 적은
  카드에 색 덩어리를 붙이면 목록의 리듬이 깨진다.

## [0.10.1] - 2026-09-05

### Fixed

- **글쓰기 화면에서 헤더가 발행하기 버튼 바를 가리던 문제.** 원인이 두 겹이었다.
  - 폼 상단 툴바가 `position: sticky; top: 0`이라 화면 맨 위, **즉 고정된 사이트 헤더 뒤에**
    붙는다(헤더 z-40 > 툴바 z-30). 캡슐이 아니어도 스크롤하면 가려지는 구조였고, 떠오르는
    캡슐이 그걸 눈에 띄게 만든 것뿐이다. 헤더가 자기 높이를 실측해 `:root`에 `--header-h`로
    심고, 툴바가 `top: var(--header-h, 67px)`로 그만큼 내려간다(하드코딩이 아니라 실측값).
  - `/posts/write`와 `/posts/*/edit`에서는 **스크롤 리스너를 아예 걸지 않는다.** 조건부로
    클래스만 빼면 물방울 모션이 진입 시 한 번 재생돼 헤더가 출렁인다 — `has-motion`도 함께
    막는다.

  일반 페이지의 캡슐 동작에는 영향이 없다(`/about`에서 `is-condensed has-motion` 확인).

## [0.10.0] - 2026-09-05

### Added

- **시리즈 안의 글 순서를 직접 바꿀 수 있다.** 지금까지 순서는 `publishedAt`에서 파생될 뿐
  저장된 순서라는 개념이 없었다. 시리즈 상세에서 로그인 상태일 때만 "순서 바꾸기"가 보이고,
  ↑↓로 옮긴 뒤 한 번에 저장한다.
  - 저장은 글마다 `seriesOrder` 번호(`models/Post.ts`). 시리즈 문서에 글 ID 배열을 두는 방식은
    쓰지 않았다 — 글이 추가·삭제될 때마다 동기화해야 하고, 그건 이 저장소가 의도적으로 피해온
    구조다. 기존 글은 전부 `null`이라 **아무도 순서를 바꾸기 전까지 동작이 그대로**고 마이그레이션도
    필요 없다.
  - **Mongo의 `.sort()`는 null을 맨 앞으로 보낸다.** 그대로 뒀으면 순서를 정해둔 시리즈에 새 글이
    들어올 때 그 글이 1편으로 튀어 올랐을 것이다. `lib/series.ts#compareSeriesPosts()`가 null을
    맨 뒤로 보내는 단일 규칙이고, 순서를 쓰는 **세 곳이 전부 이걸 쓴다** — 시리즈 상세 목록,
    글 상세의 이전/다음(`lib/posts.ts#getSeriesNav`), 목록 카드의 목차 미리보기(집계는 `$ifNull`로
    같은 규칙을 흉내 낸다). 하나라도 어긋나면 화면마다 다른 순서를 말하게 된다.
  - 저장 액션(`app/series/[slug]/actions.ts`)은 넘어온 slug 집합이 그 시리즈의 published 글 집합과
    **정확히 일치할 때만** 저장한다. 저장하는 사이 다른 탭에서 글이 추가·삭제되면 일부만 번호가
    붙어 null이 섞이고, 그게 위의 정렬 함정을 다시 만든다. 실패는 `throw`가 아니라 `{ error }`로
    돌려준다(Next가 프로덕션에서 throw를 일반 메시지로 덮는다).
  - 소유자 판별은 `useSession()`(클라이언트). 페이지에서 `auth()`를 부르면 쿠키를 읽어
    `/series/[slug]`가 통째로 동적이 된다 — `PostOwnerActions`가 같은 이유로 쓰는 패턴이다.
  - 검증: DB에 순서를 직접 넣어 뒤집었을 때 세 곳이 모두 역순으로 일치, null이 섞였을 때 지정한
    글이 앞·나머지가 게시일 순으로 뒤에 오는 것까지 확인했다.

### Changed

- **시리즈 목록 카드에 목차와 마지막 업데이트를 넣었다.** 기존 카드는 "키커·제목·설명" 세 줄이
  전부였는데, 확인해보니 **시리즈 4개의 `description`이 전부 빈 문자열**이라 실질적으로 제목
  하나뿐이었다. 그게 밋밋함의 진짜 원인이었다.
  - 앞 3편 제목을 번호와 함께 보여주고(넘치면 "… 외 N편"), 왼쪽 액센트 괘선·우상단 편수·하단
    마지막 업데이트 날짜를 붙였다. 번호 뱃지는 시리즈 상세가 쓰는 원형 뱃지를 22px로 줄인
    것으로, 목록과 상세가 같은 모티프로 이어진다.
  - 둥근 모서리는 쓰지 않았다. 이 디자인 시스템은 `--radius-md: 2px`인 각진 편집 스타일이고,
    헤더의 둥근 유리는 떠 있는 내비 레이어에 한정한 예외다.
  - 설명이 비어 있으면 문단을 렌더하지 않는다(빈 `<p>`가 공백만 만들었다). 나중에 채우면 살아난다.
  - 정렬을 slug 오름차순 → **최신 업데이트 내림차순**으로 바꿨다. 카드에 날짜를 띄우는데 순서가
    날짜와 무관하면 어색하다. `localeCompare`는 쓰지 않는다(SSR 하이드레이션 불일치).

### Fixed

- **모바일에서 검색 모달과 메뉴가 동시에 열릴 수 있던 문제.** 둘 다 화면 위쪽을 덮는 데다 검색
  덮개가 전체 화면을 먹어서, 겹치면 뒤의 메뉴는 보이지도 눌리지도 않았다. 이제 한쪽을 열면 다른
  쪽이 닫힌다 — 다만 즉시 사라지는 게 아니라 각자의 퇴장 애니메이션을 재생한 뒤 언마운트된다.
  검색 버튼·햄버거·Cmd+K 세 경로 모두 확인했다.

## [0.9.1] - 2026-09-03

### Fixed

- **아이폰에서 유리가 "그냥 흐린 판"으로 보이던 문제** (사용자 실기기 보고: "모바일 크롬에서는
  뒤가 비치는 게 아니라 그냥 블러처리만 된다").
  - 원인은 버그가 아니라 iOS의 구조적 제약이다. **iOS에서는 크롬도 사파리 엔진(WebKit)으로
    돈다** — 애플이 App Store 정책으로 브라우저 엔진을 강제해 왔고, EU 규제로 대안 엔진이
    열린 뒤에도 실제로 Blink를 출시한 벤더가 없다. 즉 아이폰에서는 어떤 브라우저를 써도
    `backdrop-filter`에 물린 SVG 필터가 동작하지 않는다. `useLiquidLens`가 정확히 `false`를
    반환해 의도한 폴백이 걸린 것이고, **그 폴백이 너무 밋밋했던 것**이 진짜 문제였다.
  - 폴백을 유리답게 고쳤다: 흐림 20px → 10px(뒤 형태·색이 비치도록), 채도 190% · 대비 1.08
    (유리를 통과한 느낌), 좌우 가장자리에 안쪽으로 모이는 빛(`inset ±7px 14px -9px`)을 넣어
    두께가 있는 유리로 읽히게 했다. 틴트도 맑음(34%)과 서리(68%) 사이인 `--glass-bg-fallback`
    (라이트 52% · 다크 56%)으로 분리했다.
  - 크롬에서 `has-lens`를 떼어내 WebKit 경로를 시뮬레이션하고 A/B로 확인했다 — 같은 줄무늬
    배경에서 이전에는 거의 완전히 지워졌고, 지금은 색과 리듬이 부드러운 띠로 비친다.
  - `:not(.has-lens)`로 분리해 **굴절이 되는 환경에는 영향이 없다.** 캡슐·검색 모달·모바일
    메뉴·맨 위로 버튼 넷 모두에 적용된다.
