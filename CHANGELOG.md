...
- **네비게이션 바를 스크롤 축소형으로 개편** (사용자 요청 "navbar를 좀 트렌디하게"). 4개
  방향을 제시해 선택받았고, 표제부는 로고 크기만 키우는 안으로 확정(부제 문구 신설 없음).
  - 최상단: 2행 표제부(로고 26px + 아이콘 / 내비), 높이 112px, 불투명 배경.
    스크롤 후: 1행 글래스 바, 높이 57px, 로고 15px. 글래스 색값은 기존 `scrolled` 인라인
    스타일을 그대로 옮긴 것으로 재설계하지 않았다.
  - 레이아웃·상태 전이를 전부 `app/globals.css`의 클래스로 옮겼다. `Header.tsx`가 인라인
    스타일을 쓰고 있어 `:hover`도 미디어쿼리도 걸 수 없었던 게 아래 두 결함의 원인이다.

### Fixed

- **활성 내비 항목이 굵어지며 옆 항목을 밀던 문제.** 활성 표시가 색 + `fontWeight: 600`
  뿐이라 글자 폭이 변했고, 라우트를 옮길 때마다 내비 전체가 미세하게 흔들렸다. 굵기는
  고정하고 색 + 하단 2px 액센트 밑줄(`::after`)로만 구분한다. 브라우저에서 `/about` →
  `/posts` 이동 시 네 항목의 `getBoundingClientRect().width`가 28.2/16.1/40.3/52.4로
  전혀 변하지 않음을 확인했다.
- **내비 링크에 호버 상태가 없던 문제.** 인라인 스타일이라 애초에 걸 수 없었다. 이제
  호버 시 밑줄이 흐리게 미리 나타난다.
- **스크롤 임계값이 단일값(`scrollY > 8`)이라 경계에서 떨리던 문제.** 배경만 바뀌던
  예전에는 티가 나지 않았지만 레이아웃이 2행↔1행으로 바뀌면 스크롤을 조금만 움직여도
  왕복한다. 펼침→축소 48px, 축소→펼침 12px로 이력을 뒀다. `MutationObserver`로 클래스
  변화를 세어, `y`를 10→50→20→45→5로 오갈 때 정확히 2회(50에서 축소, 5에서 펼침)만
  바뀌는 것을 확인했다.

### 구현 노트

- **스페이서는 축소 높이를 따라가면 안 된다.** 헤더가 112 → 57로 줄 때 스페이서까지
  줄이면 문서 맨 앞에서 55px이 사라져 본문 전체가 위로 튄다. 크로뮴은 스크롤 앵커링이
  이를 가려주지만 사파리에는 그 기능이 없다. 정작 스페이서가 필요한 지점은 스크롤 0
  (=펼침)뿐이므로 `ResizeObserver` 콜백에서 `is-condensed`일 때는 갱신을 건너뛴다.
  이 이펙트가 스크롤 이펙트보다 먼저 실행되므로 마운트 시 측정값은 항상 펼침 높이다.
- **모바일 햄버거 패널을 `<header>` 안으로 옮겼다.** 예전에는 `position: fixed` +
  `top: spacerH`로 헤더 높이를 재사용했는데, 위 변경으로 `spacerH`가 펼침 높이에 고정되면서
  축소 상태에서 55px 어긋나게 됐다. 헤더의 자식으로 두고 `position: absolute; top: 100%`를
  주면 헤더 높이를 자동으로 따라간다(헤더가 `fixed`라 그 자체가 컨테이닝 블록). 축소
  상태에서 헤더 하단 57px, 패널 상단 56px로 붙는 것을 확인했다.
- **2행 → 1행 전환은 보간할 중간값이 없는 불연속 변화라 `transition`으로 덮을 수 없다.**
  두 상태에 서로 다른 `animation-name`(`navSettleDown`/`navSettleUp`)을 줘 클래스 토글마다
  짧은 페이드+슬라이드가 재생되게 했다. 이름이 달라야 재생된다 — 같은 이름이면 조상
  클래스만 바뀌어서는 애니메이션이 다시 시작하지 않는다.
- 데스크톱 내비와 모바일 패널 링크가 `linkStyle()` 하나를 공유하던 것을 `.nav-link` /
  `.nav-link-mobile`로 분리했다. 한쪽만 손대면 다른 쪽이 같이 바뀌던 구조였다.
